...
module.exports = {
 ...
 async onRun({ api, event, fonts, args }) {
  try {
    ...
    const message = `${fonts.bold(`Group Chat`)}: ${fonts.mono(`${groupInfo.threadName}`)} ━━━━━━━━━━━━━━━\n\n${fonts.bold(`Member`)}: ${memberCount}${groupPicture ? `\n${fonts.bold(`Group Picture`)}: ${groupPicture}` : ''}`;
