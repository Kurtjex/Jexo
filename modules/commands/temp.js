...
onRun: async function({ api, args, event }) {
    ...
    if (command === "gen") {
      try {
        ...
        return api.sendMessage(`✉️ | 𝗀𝖾𝗇𝖾𝗋𝖺𝗍𝖾𝖽 𝖾𝗆𝖺𝗂𝗅: ${email}`, event.threadID);
      } catch (error) {
        console.error(error);
   ...
   try {
      ...
      return api.sendMessage("❌ | Invalid command. Use (gen) or (inbox) [email].", event.threadID);
    }
  }
  });
